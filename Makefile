...
# bun install must have run before gen: buf.gen.yaml's TS plugins are
# web/node_modules/.bin/protoc-gen-es and protoc-gen-connect-query, which only
# exist once web deps are installed. Run `make bootstrap` first on a fresh
# clone.
